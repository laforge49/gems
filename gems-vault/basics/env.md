...
The environment is an immutable hash map, keyed by [id](basics/id.md) [keyword](basics/keyword.md)s and which holds all parameters and state. The environment is in turn held by a local variable rather than being global. The contents of this map are often modified and the new versions are passed as arguments when calling functions in the gems framework. 
